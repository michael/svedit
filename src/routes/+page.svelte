<script>
  ...
  let entry_session = new EntrySession({
    ...
    body: [
      { type: 'story', layout: 1, image_width: 300, image_height: 300, image: '/images/undraw_happy_feeling_re_e76r.svg', title: ['In-place editing', []], description: ['Model your content in JSON, render it with Svelte components, and edit content directly in the layout. You only have to follow a couple of rules to make this work.', []] },
      { type: 'story', layout: 2, image_width: 300, image_height: 300, image: '/images/container-cursors.svg', title: ['Container cursors', []], description: ['They work just like text cursors, but instead of a character position in a string they address a block position in a container.\n\nTry it by selecting a few blocks, then press ↑ or ↓. Press ↵ to insert a new block or ⌫ to delete the block before the cursor.', []] },
      { type: 'story', layout: 1, image_width: 300, image_height: 300, image: '/images/lightweight.jpg', title: ['Lightweight', []], description: ['The reference implementation uses only about 1000 lines of code. That means you\'ll be able to serve editable web pages, removing the need for a separate Content Management System.', []] },
      { type: 'story', layout: 2, image_width: 300, image_height: 300, image: '/images/nested-blocks-illustration.svg', title: ['Nested blocks', []], description: ['A block can embed a container of other blocks. For instance the list block at the end has a container of list items.', []] },
      // -- 
      { type: 'story', layout: 1, image_width: 300, image_height: 300, image: '/images/undraw_undraw_undraw_undraw_undraw_undraw_undraw_undraw_users_per_minute_1e4q_t22j_-1-_0ngf_-1-_27dv_30ul_legv_-1-_il1l_-2-_0jip.svg', title: ['Alpha version', []], description: ['Expect bugs. Expect missing features. Expect the need for more work on your part to make this work for your use case.\n\n Find below a list of known issues we\'ll be working to get fixed next: ', []] },
      {
        type: 'list',
...
<div class="demo-wrapper pbs-6">
  ...
  <Svedit {entry_session} editable={true} class='flex-column gap-y-10'>
    ...
    <div contenteditable="false" style="background: #eee; opacity: 0.5; padding: 20px;">
      <div>While in this example title above is editable, this piece of content here is not. Below is a container of Story and List blocks:</div>
    </div>
